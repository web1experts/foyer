        ...
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="javascript:void(0)" type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#insertModal"><?= lang('add_company') ?></a>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel"><?= lang('add_company') ?></h5>
            ...
            <div class="modal-body">
                ...
                <form method="post" id="quickForm" enctype="multipart/form-data" class="insert_data">
                    <div class="form-group">
                        <label for="title"><?= lang('cmpTitle') ?></label>
                        <input type="text" name="slider_text" class="form-control" placeholder="<?= lang('cmpTitle') ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="title"><?= lang('cmpsubTitle') ?></label>
                        <input type="text" name="cmp_nick_title" class="form-control" placeholder="<?= lang('cmpsubTitle') ?>" required>
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="post" id="quickForm" enctype="multipart/form-data" class="update_data">
                    <div class="form-group">
                        <label for="title"><?= lang('cmpTitle') ?></label>
                        <input type="text" id="banner_text" name="banner_text" class="form-control" placeholder="Title" required>
                    </div>

                    <div class="form-group">
                        <label for="title"><?= lang('cmpsubTitle') ?></label>
                        <input type="text" id="cmp_nick_title" name="cmp_nick_title" class="form-control" placeholder="Tab Label" required>
                    </div>
    ...
    $(document).ready(function() {
        ...
        function get_datatable() {
            $('#data_table').DataTable({
                ...
                'iDisplayLength': 100,
                "ajax": {
                    "url": "<?php echo base_url('/bannercontroller/data_cmptable') ?>",
                    "type": "POST"
                },
                "columnDefs": [{
                    "targets": [0],
        ...
        $.validator.setDefaults({
            ...
            submitHandler: function() {
                $.ajax({
                    url: '<?php echo base_url('/bannercontroller/save_slider'); ?>',
                    type: "POST",
        ...
        $("body").on("click", ".user_action", function() {
            ...
            if (get_title == "Edit") {
                ...
                $.ajax({
                    url: '<?php echo base_url('/bannercontroller/edit_banner'); ?>',
                    type: "POST",
                    data: {
                    ...
                    success: function(resp) {
                        ...
                        $("#banner_text").val(resp.cmp_text);
                        $("#cmp_nick_title").val(resp.cmp_nick_title);
                        $("#banner_desc").val(resp.cmp_desc);
                        $("#edit_id").val(edit_id);
                        $("#current_image").attr('src', resp.thumbnail);
                        // $("#editModal input[name=icon_path]").val(resp.cmp_image);
                        // $("#editModal input[name=icon_thumb]").val(resp.cmp_thumb);
        ...
        $.validator.setDefaults({
            ...
            submitHandler: function() {
                $.ajax({
                    url: '<?php echo base_url('/bannercontroller/update_banner'); ?>',
                    type: "POST",
