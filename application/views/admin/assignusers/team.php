...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <p class="text-dark">Team: <?= (isset($selected_team) && !empty($selected_team))?$selected_team->name:''; ?></p>
                    <h1><?= lang('users') ?></h1>
                </div>
        ...
        function get_datatable() {
            $('#data_table').DataTable({
                ...
                "ajax": {
                    ...
                    "type": "POST",
                    "data": {
                        'company_id':"<?php echo $_REQUEST['team'] ?>",
                ...
                if (willDelete) {*/                    
                    $.ajax({
                        ...
                        success: function(resp){
                            if(resp.success){
                                if(get_type=="remove"){
                                    $this.removeClass('btn-danger').addClass("btn-success").attr('data-title', 'add').html("Assign to Team");
                                } else {
                                    $this.removeClass('btn-success').addClass("btn-danger").attr('data-title', 'remove').html("Remove from Team");;
                                }
                                swal(resp.success, {
                                    icon: "success",
                                });
