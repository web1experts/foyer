...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<p class="text-dark">User: <?= (isset($selected_user) && !empty($selected_user)) ? $selected_user->user_fname . ' ' . $selected_user->user_lname : ''; ?></p>
							...
							<div>
								<nav class="navbar navbar-expand-lg">
									...
									<div class="collapse navbar-collapse" id="navbarSupportedContent">
										...
										<a href="javascript:;" id="insert-new-tab" class="insert_modalbox" data-toggle="modal" data-target="#inserttabModal"><i class="fa fa-plus"></i></a>
					...
					<div class="col-md-12">
						<div id="card-group" class="card-group desktop_bookmark">
							<div class="card wow zoomIn unsortable">
								<a id="bookmark" class="insert_modalbox" href="javascript:void(0)">
...
<div class="modal fade" id="insertbookmartModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content" style="width: 100%;">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel"><?= lang('assignBookmarks') ?></h5>

				<button type="button" id="close_modal" onclick="tab_refresh()" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
...
<script type="text/javascript">
	$(function() {
					...
					$.ajax({
						...
						success: function(resp) {
							...
							if (resp.success_status) {
								...
								if (data_target !== '#insertbookmartModal') {
									$("ul#tabs-list").append('<li class="nav-item bookmark_nav ui-sortable-handle"><a class="nav-link bookmark_tabs" title="' + title + '" data-id="' + resp.last_id + '" data-title="tabs" href="javascript:;">' + subtext + '</a></li>');
								} else {
		...
		$("body").on("click", "#bookmark", function() {
			...
			var get_type = $(".bookmark_tabs li.active").attr('data-type');
			var get_id = $(".bookmark_tabs li.active").attr('data-id');
			...
			setTimeout(function() {
				var data_table = $('#data_table').DataTable({
					...
					"order": [],
					"ajax": {
						"url": "<?php echo base_url('/admincontroller/assign_bookmark_dataTable_tabs') ?>",
						"type": "POST",
						"data": {
							'table_type': get_type,
							'table_id': get_id,
							'user_id': "<?php echo base64_decode($_REQUEST['user_id']); ?>"
						},
					},
					"columnDefs": [{
						"targets": [0],
			...
			$.ajax({
				...
				success: function(resp) {
					if (resp.code == 200) {
						...
						booksmarkssort();
						$(".desktop_bookmark").append('<div  class="card wow zoomIn unsortable"><a id="bookmark" class="insert_modalbox" href="javascript:void(0)"><img class="card-img-top" src="' + ajax_url + '/assets/images/plus.png"></a></div>');
					} else {
						Swal.fire({
							title: "Error!",
							text: resp.message,
						...
						$(".desktop_bookmark").html('<div  class="card wow zoomIn unsortable"><a id="bookmark" class="insert_modalbox" href="javascript:void(0)"><img class="card-img-top" src="' + ajax_url + '/assets/images/plus.png"></a></div>');
					}
				},
				error: function() {
		...
		$("body").on("click", ".admin_action", function() {
			...
			var get_nm = $this.attr('data-nm');
			var get_dataTitle = $this.attr('data-title');


			var active_type_id = $(".bookmark_tabs li.active").attr('data-id'); //Team,compant and user ID
			...
			swal({
				...
				.then((willDelete) => {
					if (willDelete) {
						$.ajax({
							url: "<?php echo base_url('/admincontroller/update_bookmark') ?>",
							type: "post",
							dataType: "json",
							data: {
								'type': get_type,
								'type_id': get_value,
								'type_title': get_dataTitle,
								'active_tab_id': active_type_id
							},
							success: function(resp) {
								if (resp.success) {
									if (get_dataTitle == "remove") {
										var second_message = "Click to assigned " + get_nm + " bookmark for team";
										$this.removeClass('btn-danger').addClass("btn-success").attr('data-title', 'add').attr('title', second_message).html("Assign");
									} else {
										var second_message = "Click to unassigned " + get_nm + " bookmark for team";
										$this.removeClass('btn-success').addClass("btn-danger").attr('data-title', 'remove').attr('title', second_message).html("Unassigned");;
									}
									swal(resp.message, {
										icon: "success",
									});
