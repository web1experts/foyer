    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><?= lang('faq') ?>
                    <a href="javascript:void(0)" type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#insertModal"><?= lang('add_faq') ?></a></h1>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><?= lang('add_faq') ?></h5>
      ...
      <div class="modal-body">
        ...
        <form method="post" id="quickForm" enctype="multipart/form-data" class="insert_data">
            <div class="form-group">
                <label for="title"><?= lang('questions') ?></label>                
                <textarea class="form-control" name="faq_qst" placeholder="<?= lang('questions') ?>" required></textarea>
            </div>


            <div class="form-group">
                <label for="image"><?= lang('answer') ?></label>
                <textarea class="form-control" name="faq_ans" placeholder="<?= lang('answer') ?>" required></textarea>
            </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form method="post" id="quickForm" enctype="multipart/form-data" class="update_data">
            <div class="form-group">
                <label for="title"><?= lang('questions') ?></label>                
                <textarea class="form-control" name="edit_faq_qst" id="edit_faq_qst" placeholder="<?= lang('questions') ?>" required></textarea>
            </div>


            <div class="form-group">
                <label for="image"><?= lang('answer') ?></label>
                <textarea class="form-control" name="edit_faq_ans" id="edit_faq_ans" placeholder="<?= lang('answer') ?>" required></textarea>
            </div>
    ...
    $(document).ready(function () {
        ...
        function get_datatable() {
            $('#data_table').DataTable({
                ...
                "order": [],
                "ajax": {
                    "url": "<?php echo base_url('/faqscontroller/data_table_faqs') ?>",
                    "type": "POST"
                },
                "columnDefs": [{
                        "targets": [0],
        ...
        $.validator.setDefaults({           
            ...
            submitHandler: function () {
                $.ajax({
                    url: '<?php echo base_url('/faqscontroller/save_faqs'); ?>',
                    type: "POST",
        ...
        $("body").on("click", ".user_action", function(){
            ...
            if(get_title=="Edit"){
                ...
                $.ajax({
                    url: '<?php echo base_url('/faqscontroller/faqs_edit'); ?>',
                    type: "POST",
                    ...
                    beforeSend: function (xhr) {
                        ...
                        $("#edit_faq_qst").val(resp.faq_question);
                        $("#edit_faq_ans").val(resp.faq_answer);
                        $("#edit_id").val(resp.faq_id);                        
        ...
        $.validator.setDefaults({           
            ...
            submitHandler: function () {
                $.ajax({
                    url: '<?php echo base_url('/faqscontroller/update_faqs'); ?>',
                    type: "POST",
